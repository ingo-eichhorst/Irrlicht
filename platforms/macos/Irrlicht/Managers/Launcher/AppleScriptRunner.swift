...
enum AppleScriptRunner {
    ...
    // hardcoded template (see TerminalAppActivator/ITermActivator) and only
    // interpolates values that went through `escape()` above into a
    // quoted-string-literal position — never into a position that could
