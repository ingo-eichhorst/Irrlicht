...
/// #1874, by what the reference set actually contains rather than by a grep:
/// the directories under `Tests/__Snapshots__`
/// (`find Tests/__Snapshots__ -mindepth 1 -type d | wc -l` → 6, holding
/// `find Tests/__Snapshots__ -name '*.png' | wc -l` → 60 references) are
/// `DaemonErrorBannerRenderTests`, `GroupViewSnapshotTests`,
/// `HistoryViewSnapshotTests`, `PermissionWizardEffectErrorRenderTests`,
/// `SessionRowSnapshotTests` and `UnappliedGrantsBannerRenderTests` — rows,
/// groups, history, the wizard and the two banners, none the panel header. So
/// that reference set is untouched by construction rather than by inspection.
///
/// An earlier revision of this comment cited
/// `git grep -n "rateLimit" -- platforms/macos/Tests` as "matches only a doc
/// comment". That is false and was false when written — it matches 18 times
/// across four files (`MenuBarFixtures`, `PinnedNowSnapshot`,
/// `QuotaChipClockTests`, `QuotaMenuBarRendererTests`). The conclusion above
/// never rested on it; the enumeration does, and it is checkable.
///
