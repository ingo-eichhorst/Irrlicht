#!/usr/bin/env bash
# provapi-bounded-read-mutations_test.sh — committed mutation fixture 2/5 for
# issue #2003's account-quota transport (core/adapters/outbound/accountquota).
#
...
#
# The mutation removes readBounded's io.LimitReader wrap
# (`limited := io.LimitReader(r, responseSizeCeiling+1)` becomes `limited :=
# r`), reproducing #2003 §1.2's "an unbounded io.ReadAll is not acceptable".
# TestReadBounded_NeverPullsPastTheCeilingPlusOne is written to catch the
# READ ITSELF being unbounded, not merely the size classification that
# follows it — a test that only checked the returned error would pass
# unchanged under this mutation, because len(data) > responseSizeCeiling
# still classifies an oversized body correctly either way. What differs is
# how many bytes were pulled from the source to get there, which is what a
# countingReader measures directly.
#
...
need() {
  ...
  if ! command -v "$tool" >/dev/null 2>&1; then
    echo "FAIL: provapi-bounded-read-mutations — $tool not found" >&2
    exit 1
  fi
  return 0
}
...
if [[ ! -x "$MUTATE_SH" ]]; then
  echo "FAIL: provapi-bounded-read-mutations — $MUTATE_SH is missing or not executable" >&2
  exit 1
fi

if [[ -n "$(git -C "$REPO_ROOT" status --porcelain)" ]]; then
  echo "provapi-bounded-read-mutations: CANNOT RUN — the worktree is dirty, and mutate.sh needs a" >&2
  echo "  clean tree for its post-restore check to mean anything. Commit or clean up, then:" >&2
  echo "    bash tools/lib/provapi-bounded-read-mutations_test.sh" >&2
...
# ── readBounded's LimitReader wrap is removed ──────────────────────────────
assert_go_test_goes_red \
  "removing the LimitReader wrap lets readBounded pull the whole body regardless of size" \
  ...
  "TestReadBounded_NeverPullsPastTheCeilingPlusOne" \
  "the read itself is unbounded"

if [[ $fails -gt 0 ]]; then
  echo "provapi-bounded-read-mutations: $fails FAILED"
  exit 1
fi
echo "provapi-bounded-read-mutations: ALL PASS"
