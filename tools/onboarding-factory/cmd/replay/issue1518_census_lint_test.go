...
var coincidentalCensusFigures = []censusFigureExemption{
	...
	// The entry that used to sit here — timing_drift.go's "1-10ms" bucket,
	// shadowing Recordings — is deleted. Promoting cell 2-12's Desktop
	// recording moved Recordings one further along that same histogram, off
	// the row it had landed on, and this check reported the entry as
	// suppressing nothing in the same run. Third direction of the #1388
	// behaviour in this file: a figure walking OFF a row it had walked ONTO
	// two commits earlier.
	{
