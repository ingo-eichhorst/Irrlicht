...
var knownFabricated = map[string]string{
	...
	"muse/scenarios/2-14_turn-aborted-by-error/recordings/2026-09-14-06-16-22_irrlichd-0.6.3+a46599a/transcript.jsonl":         "the SAME #1960 root cause as the 2-17 entry above, its deterministic (not merely racy) manifestation: this is the pre-fix attempt, captured against irrlichd-0.6.3+a46599a (predates fix commit 9abf13d1). The daemon's own assignPIDLocked deleted the real session outright when a proc-<PID> pre-session claimed its already-bound PID, so the daemon's raw log shows 0 transitions for the rest of the session's ~8s life despite 169KB+ of tailed transcript_activity. The offline replay sidecar re-derives the 2 real transitions (ready->working->error) from the same transcript bytes, which is why its golden disagrees with what the daemon actually logged live. Superseded by the post-fix recording at 2026-09-14-06-40-20_irrlichd-0.6.3+9abf13d (commit 990daf13), kept as committed evidence of the pre-fix behavior rather than deleted (replaydata recordings are not hand-edited or removed once committed).",
}
...
// replayed >= recorded generally. A large minority of the catalog still shows
// milder extended-check divergence (dominant kind: a missing terminal
...
// The size of that population is deliberately NOT restated here — it is
// censusOfTheCommittedCatalog.Divergent, machine-generated by
// TestCatalogCensusMatchesTheCommittedFigures. A hand-typed copy in this
// comment is precisely what #1503 removed: this one said 140 while
// tools/replay-fixtures.sh said 198.
func TestSidecarReplayAgreesWithTheDaemonsOwnLog(t *testing.T) {
	...
	checked := forEachSidecarRecording(t, func(name string, ec *extendedCheck) {
		switch {
		// Both arms read the named predicates rather than respelling them
		// inline: these two counts are quoted in doc comments and PR tables,
		// and #1503's whole finding is that a figure with more than one
		// definition acquires more than one value (see extendedCheck.Diverges).
		case ec.ReproducesNothing():
			seenZero[name] = true
