...
// sidecarBuckets holds the timeline streams extracted from a sidecar for
...
func (r *sidecarReplayer) classifyAt(fileSize int64, ctx transitionCtx) error {
	...
	// every other call site in this file (#1360). The daemon turns an
...
func (r *sidecarReplayer) applyProcessTeardown(entry timelineEntry, d *debounceState) error {
	...
	// it up before tearing down, exactly as the hook and orphan branches do
	// (issue #1342). Without this, a recording whose fs events all land inside
	// one debounce window replayed ZERO transitions: the single classify pass
	// ran on the first event against a partial transcript, every later event
	// only extended the window, and this branch then discarded it — leaving a
	// golden that held nothing but the synthetic init row and so could not fail
	// when the classifier regressed. It also explains why relaxing
	// flushPendingDebounce's early-return measured as no-change: by the time
	// that trailing flush is reached, the reset below has already zeroed
	// d.pending, d.coalesced and d.pendingSize, so there is nothing left to fire.
