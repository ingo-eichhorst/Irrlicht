...
// with one agent in `scenarios/` and one in `regressions/`, and asserts
...
func TestScenariosList_walksReplaydataTree(t *testing.T) {
	...
	mkRecording(t, root, "aider", "regressions", "llm-error",
	...
	if entries[0].Agent != "aider" || entries[0].Subtree != "regressions" {
