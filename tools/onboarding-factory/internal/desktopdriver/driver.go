...
// startSession opens a composer and records a new, UNBOUND slot. It does not
// wait for an owned Desktop session here: Claude Desktop writes no
// claude-code-sessions registry row until the first message is sent (see the
// comment on effectiveScript), so a composer nothing has been typed into yet
// has no row for WaitOwnedSession to find. The slot is bound by whichever
// `send` step targets it first — see sendFirst.
func (runner *scriptRunner) startSession(ctx context.Context, workspace string) error {
	...
	runner.slots = append(runner.slots, &ownedSlot{workspace: workspace})
	runner.active = len(runner.slots) - 1
	return nil
}

// adopt binds a newly owned Desktop identity to a slot that startSession left
// unbound, and refuses to bind the same identity to two slots — that would
// make the teardown archive it twice and the identity table lie about how
// many sessions ran.
func (runner *scriptRunner) adopt(slot *ownedSlot, owned OwnedSession) error {
	for index, other := range runner.slots {
		if other == slot {
			continue
		}
		if other.owned.Registry.SessionID == owned.Registry.SessionID {
			return fmt.Errorf(
				"Desktop session %q was already adopted as slot %d; the driver cannot own one session twice",
				owned.Registry.SessionID, index+1)
		}
		if owned.Transcript.SessionID != "" && other.owned.Transcript.SessionID == owned.Transcript.SessionID {
			return fmt.Errorf(
...
// send types and submits one turn against the active slot. A slot's FIRST send
// binds its ownership — see sendFirst — so this only drives the ready-wait,
// type, and submit sequence for a slot that already has an owned session.
...
// 2026-09-05, an open, trusted composer showing the right Local project
// produced no row for 36 seconds — so waiting for one any earlier can never
// observe it. There is also no pre-turn "ready" to wait for: a slot that has
...
func (runner *scriptRunner) sendFirst(ctx context.Context, slot *ownedSlot, text string) error {
	...
	if err := runStep(ctx, runner.request.StepTimeout, "one owned Desktop session", func(step context.Context) error {
		var err error
		owned, err = runner.runtime.WaitOwnedSession(step, runner.claimedBaseline(), slot.workspace)
		return err
	}); err != nil {
		return err
	}
