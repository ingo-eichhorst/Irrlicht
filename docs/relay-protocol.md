...
| --- | --- | --- |
| `GET /api/v1/push/info` | none | `{"enabled":false,"reason":"…"}` or `{"enabled":true,"vapid_public_key":"…","public_url":"https://…"}`. `public_url` is present only when `--public-url` is a valid HTTPS origin; otherwise `pairing_url_reason` explains why QR pairing is unavailable. Feature detection: clients render the pairing UI only where this answers `enabled:true`, so an old relay (404) and an anonymous one hide it. |
| `POST /api/v1/push/pairings` | client token | `201 {"code":"XXXX-XXXX","expires_in":600,"pairing_url":"https://…/pair/XXXX-XXXX/","pairing_qr":"data:image/png;base64,…"}` — a single-use code in **the caller's own workspace**. The URL and QR fields are omitted when no valid public URL is configured; `pairing_url_reason` then names the manual fallback. `429` past 32 outstanding codes. |
| `GET /pair/<code>/` | code in path | Install handoff page. Its manifest uses the stable app id and root scope, with this path as the start URL. The installed app carries the code into the root pairing UI and waits for the user to press **Pair this phone**. Invalid code shapes return `404`; validity and expiry are still decided only by `POST /api/v1/push/pair`. |
