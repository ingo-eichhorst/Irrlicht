...
gh issue list --repo ingo-eichhorst/Irrlicht --state open --limit 200 \
  ...
  | jq -r '.[] | select(
      ...
      any(. == "needs-triage" or . == "needs-info" or
          . == "ready-for-agent" or . == "wontfix") | not
    ) | .number'
```
...
1. **Read.** `gh issue view <N> --repo ingo-eichhorst/Irrlicht --comments`. If a prior `*This was generated by AI during triage.*` comment already exists and a state label is already applied, skip — already triaged.
...
| **Independence** | Not blocked on another issue / PR / external decision. No concurrent work that would conflict. | Depends on #N which is open / unblocked / unmerged; depends on a maintainer decision not yet made. |
...
Industry context: SWE-Bench Pro shows top agents resolve ≤23% of "real-world" tasks vs. >70% of curated tasks. The gap is patch size and ambiguity. Calibrate accordingly:
...
Two states. Plus `wontfix` for closure.
