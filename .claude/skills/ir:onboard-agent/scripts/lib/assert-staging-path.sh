...
# the staging dir at replaydata/ (or the legacy testdata/). Sourced (not
# exec'd) so it can fail-fast in the caller's process.
#
# Caller must set $STAGING and $REPO_ROOT.

if [[ "$STAGING" != "$REPO_ROOT/.build/refresh/"* ]] || [[ "$STAGING" == *"/replaydata/"* ]] || [[ "$STAGING" == *"/testdata/"* ]]; then
