...
> Baseline established 2026-08-05 with the adapter itself (#1256). Repo `github/copilot-cli`, adapter name `copilot`. No release sweep has been run against this agent yet — everything below is what the onboarding established about the *current* build, so it is the baseline a first sweep should diff against, not a set of release findings.

- Verified against CLI **1.0.77** (assessment) and **1.0.78** (the recording sweep). The version string prints with a trailing period (`1.0.77.`), which the factory's `precheck.sh` strips
- CONFIRMED (1.0.78): transcript is `events.jsonl` under `~/.copilot/session-state/<session-id>/`; root relocatable via **`$COPILOT_HOME`** (`--config-dir` is deprecated upstream in favour of it). The filename is constant, so the session id comes from the parent directory name
- CONFIRMED: every record shares one envelope `{id, parentId, timestamp, type, data}` — a new record type is additive and lands in the parser's default Skip arm rather than breaking the dispatch. That makes *silent* additions the risk, not crashes
- CONFIRMED: turn boundaries are explicit (`assistant.turn_start` / `assistant.turn_end`). **Sharp edge:** copilot closes the turn after every tool call and opens a continuation ~50-80ms later with no `user.message` between, so a raw `turn_end` count runs ahead of the number of user turns. Any future change to that continuation shape is a break surface for both the parser and the recording driver
- CONFIRMED: permission prompts are on disk (`permission.requested` / `permission.completed`, paired on `requestId`), so `waiting` needs no hook install — unlike claude-code and codex. A move of that pairing to an ephemeral channel would silently cost the adapter its headline capability
- CONFIRMED: `session.usage_checkpoint` carries **no** token counts; the cumulative `modelMetrics` block exists only on `session.shutdown`. Live tokens/cost therefore ride on per-message `assistant.message.outputTokens`. A release that moves counts into the checkpoint is an opportunity, not a break
- CONFIRMED: in-process subagents (`task`, `explore`) create **no** child session directory — their `user.message`, tool and completion records are appended to the *parent's* `events.jsonl`, distinguished only by a top-level `agentId`. `subagent.started` / `subagent.completed` pair on `toolCallId`
- CONFIRMED: `/compact` writes `session.compaction_start` / `session.compaction_complete` as ordinary persisted records; `/autopilot on` writes `session.mode_changed`, self-issues continuations as `user.message` with `agentMode: "autopilot"` and empty content, and stops at `--max-autopilot-continues` (default 5). Autopilot **requires** the `task_complete` tool to be available at every continuation — restricting `--available-tools` without it aborts the loop with `session.error{errorType: "query"}`
- CONFIRMED: error categories are first-class and named — a rate-limited turn writes `session.error{errorType: "rate_limit"}`, and in both the quota and the unreachable-provider case `assistant.turn_end` lands *before* the error, so the session settles rather than sticking in `working`
- ENVIRONMENT, NOT A RELEASE FINDING: on the account used for onboarding, `/model <concrete-id>` is refused (*"only Auto mode is available on your plan"*) and the auto-router resolves **once per session**, not per turn. A plan with individual model access would change what the model-switch scenario can observe
- WATCH: BYOK (`COPILOT_PROVIDER_BASE_URL` / `_TYPE` / `_WIRE_API` / `_API_KEY` + `COPILOT_MODEL`) bypasses GitHub's CAPI entirely and spends zero AI credits. This is what makes the quota and error-abort cells recordable for free; a change to those variable names would cost the factory two cells
- UNSETTLED: whether `esc`-`esc` cancels a *streaming* turn. Two live attempts on 1.0.78 failed to interrupt a four-minute generation, so how copilot records a real user interrupt is still unknown
...
- **opencode `message.data.error`**: non-issue. irrlicht reads the SQLite `message.data`
  ...
  what is *persisted* into `message.data` — a schema question, not an event one.
- **opencode config**: irrlicht reads no opencode config file — zero references in the
...
- **codex unknown-type tolerance**: proven on disk, not theoretical. `~/.codex/sessions`
  on this machine already contains `world_state` ×17, `inter_agent_communication_metadata`
  ×22, `compacted` ×7 — none in the parser's switch, all hitting `default: Skip`
  ...
  *fields* are structurally impossible to break: decode targets a generic
  ...
  (#1067's core paginated-thread-format watch item is unrelated and remains open — only
