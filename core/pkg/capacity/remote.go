...
func parseLiteLLMData(data []byte) (*capacityConfig, error) {
	...
	for key, rawEntry := range raw {
		...
		//
		// "mistral/" is an exception: unlike Anthropic/OpenAI, Mistral's own
		// direct API has no bare (unprefixed) canonical key in LiteLLM at
		// all — only "mistral/<model>" (its Bedrock/Azure-hosted variants use
		// their own distinct prefixes, e.g. "mistral.mistral-large-2402-v1:0"
		// with a dot not a slash, so they're unaffected here). Skipping it
		// silently dropped every Mistral model's pricing from the capacity
		// table. See modelAliases for the "mistral-medium-3.5" ->
		// "mistral/mistral-medium-3-5" bridge vibe's reported model name needs.
		if key == "sample_spec" {
			continue
		}
		if strings.Contains(key, "/") && !strings.HasPrefix(key, "mistral/") {
			continue
		}
