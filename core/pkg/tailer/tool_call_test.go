...
// --- TurnDone tracking tests ---

func TestTurnDone_SetOnTurnDoneEvent(t *testing.T) {
	path := writeTranscriptLines(t, []map[string]interface{}{
		{"type": "user", "timestamp": ts(0)},
		{"type": "assistant", "timestamp": ts(1)},
	...
	if !m.TurnDone {
		t.Error("expected TurnDone=true after turn_done event")
	}
}

func TestTurnDone_ClearedOnUserMessage(t *testing.T) {
	// turn_done followed by a new user message (new turn) → TurnDone cleared.
	path := writeTranscriptLines(t, []map[string]interface{}{
	...
	if m.TurnDone {
		t.Error("expected TurnDone=false after user message (new turn)")
	}
}

func TestTurnDone_NotSetMidTurn(t *testing.T) {
	// Between tool calls: assistant text with no turn_done → TurnDone stays false.
	path := writeTranscriptLines(t, []map[string]interface{}{
		{"type": "user", "timestamp": ts(0)},
		{"type": "assistant", "timestamp": ts(1), "message": map[string]interface{}{
			"content": []interface{}{
		...
		{"type": "tool_result", "timestamp": ts(2)},
		// Mid-turn assistant response (between tool calls)
		{"type": "assistant", "timestamp": ts(3), "message": map[string]interface{}{
			"content": []interface{}{
				map[string]interface{}{"type": "text", "text": "Let me check the next file"},
			},
		}},
	})
	...
	if m.TurnDone {
		t.Error("expected TurnDone=false mid-turn (no turn_done event)")
	}
	if m.LastEventType != "assistant" {
		t.Errorf("expected LastEventType=assistant, got %q", m.LastEventType)
	}
}

func TestTurnDone_PersistsAcrossIncrementalReads(t *testing.T) {
	// TurnDone should persist when no new events arrive (incremental reads).
	path := writeTranscriptLines(t, []map[string]interface{}{
		{"type": "user", "timestamp": ts(0)},
		{"type": "assistant", "timestamp": ts(1)},
		{"type": "system", "subtype": "turn_duration", "timestamp": ts(2)},
	...
	// First read: turn_done → TurnDone=true.
	m, err := tailer.TailAndProcess()
	if err != nil {
		t.Fatal(err)
	}
	if !m.TurnDone {
		t.Error("first read: expected TurnDone=true")
	}

	// Second read with no new events: TurnDone should persist.
	m, err = tailer.TailAndProcess()
	if err != nil {
		t.Fatal(err)
	}
	if !m.TurnDone {
		t.Error("second read: expected TurnDone=true (should persist)")
	}
}
