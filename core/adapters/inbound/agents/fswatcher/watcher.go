...
type Watcher struct {
	...
	// before maxLoggedWatchFailures capped them. Like pendingNew above, both
	// are touched only from the Watch goroutine — addExistingDirs directly,
	// addSubtree via handleEvent — so they need no lock.
	watchFailures       int
...
func (w *Watcher) addWatch(watcher *fsnotify.Watcher, dir string) {
	...
	// is neither counted nor reported. Both backends surface the bare errno
	// here (inotify_add_watch's, and kqueue's from os.Open), so this matches
	// on either platform. The window is real on both scan paths: the startup
	// walk and handleDirCreate's Stat each precede this call.
