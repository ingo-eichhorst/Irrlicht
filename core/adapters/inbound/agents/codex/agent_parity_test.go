...
func TestAgentParityWithConfig(t *testing.T) {
	...
	t.Run("Codex Config does not set optional refinements", func(t *testing.T) {
		if c.CountOpenSubagents != nil {
			t.Error("Codex should not declare a subagent counter (file-based subagents)")
		}
		if c.ComputeMetrics != nil {
			t.Error("Codex should not declare a custom metrics provider")
		}
	})
}
