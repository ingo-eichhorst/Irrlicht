...
var stateRules = []stateRule{
	...
	{
		// The agent PROCESS went away while a turn was still open (#1800) →
		// error. The first rule in the system to decide on TierProcess, a tier
		// #1288 declared and reserved.
		//
		// WHY ITS OWN ROW, rather than a `tierOf` on the session_error rule
		// below. #1798 offered either; only one of them works, and the reason
		// is about PLACEMENT rather than about the tier value:
		//
		//   - A `tierOf` would fix the recorded provenance but leave the
		//     verdict where session_error sits, BELOW the three
		//     transcript-tier waiting rules. Those read a transcript FROZEN at
		//     the instant of death: an AskUserQuestion or a permission-gated
		//     edit that was open when the process died stays open forever, so
		//     user_blocking_tool keeps firing and keeps painting a dead
		//     session `waiting`. Sending a user to answer a question in a
		//     process that no longer exists is worse than saying nothing, and
		//     is the exact silent wrongness the fourth state exists to remove.
		//   - It would also make the ladder INCONSISTENT, not merely
		//     mis-tiered: a transcript-tier rule deciding above a
		//     process-tier rule that would also have fired is precisely what
		//     TestStateRules_LadderIsTierConsistent forbids. Overstating from
		//     below is the one direction that invariant does catch, which is
		//     why this placement is enforced mechanically rather than by this
		//     comment.
		//
		// Two rows also keep the two verdicts apart in a recorded trace:
		// DecidedByRule reads "process_death" or "session_error", so a crash
		// and a provider refusal are never the same line.
		//
		// WHY HERE — below the two hook-tier rules, above everything else.
		// TierHook (1) outranks TierProcess (3), so permission_prompt and
		// compact_in_progress stay above; that is also semantically right,
		// since a hook that fired is the agent reporting on itself.
		// Everything below this point is transcript tier, which a dead
		// process outranks.
		//
		// MUTUAL EXCLUSION WITH THE TWO HOOK-TIER RULES BELOW is what keeps the
		// ladder consistent, and it is NOT spelled here as a guard — it falls out
		// of the holds' staleness rules, which are exactly complementary:
		//
		//	SignalProcessDeath  stale when  IsAgentDone()
		//	SignalIdlePrompt    stale when !IsAgentDone()
		//
		// so at most one of the two can ever be applied on a pass, and
		// agent_done — hook-tier precisely when HookTurnDone, which implies
		// IsAgentDone — is excluded by the same line. A restated `!HookTurnDone
		// && !IdlePromptPending` guard here was tried and REMOVED: with those
		// staleness rules in place neither clause could be made to fail under
		// mutation, so each was a check that did not reach what it claimed to
		// cover while reading, to the next editor, as the thing holding the
		// ladder up.
		//
		// What does hold it up is SignalProcessDeath's `stale` rule, and that IS
		// mutation-covered: neutering it fails
		// TestStateRules_LadderIsTierConsistent with
		// `rule "process_death" (tier process) decided, but lower rule
		// "agent_done" (tier hook) outranks it`. That rule also carries the
		// producer's real verdict — see its comment for why the decision cannot
		// be made on the process-exit edge at all.
		//
		// The producer's predicate is much narrower still — it converts only a
		// session that was `working` with no turn-end in sight, see
		// SessionDetector.retainAsProcessDeath — but the ladder must not rest
		// on a producer's discipline, so the guards are restated where the
		// invariant can see them.
		id:     string(session.SignalProcessDeath),
		signal: session.SignalProcessDeath,
		when: func(_ string, m *session.SessionMetrics) bool {
			return m.ProcessDeath && m.SessionError != nil
		},
		decide: toState(session.StateError, "agent process died mid-turn → error"),
	},
	{
	...
	},
	{
		...
		//
		// #1800 adds five more producers (gemini-cli, opencode, codex, pi,
		// aider), all of them transcript-only. They are not affected by the
		// hook half above — none of those adapters delivers a Stop hook — but
		// every one of them reports its failure ON the turn-boundary event
		// itself, so ClearedByTurnBoundary is the rule that decides them too.
		//
