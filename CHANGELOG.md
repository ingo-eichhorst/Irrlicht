...
#### The menu bar icon becomes a budget, not a toggle
![The macOS menu bar showing the Irrlicht icon with three project dot-groups, an overflow ellipsis for the rest, and two stacked quota bars for two separate subscriptions](assets/releases/v0.6.4/menu-bar-slots.png)

The compact on/off switch is gone. You now pick how the icon groups sessions — one bucket per project, one per machine, or everything collapsed into a single dot — and how many slots the whole icon may spend before the rest fold into one `…`. A second setting lists which subscriptions get their own quota bar, and in which order they are drawn.

**Why it matters:** the icon stops growing without limit, and you decide what its width is spent on instead of accepting one hardcoded number.
...
**Added**
- Muse — Meta's `muse` CLI, 1.2.1 and later — is a supported agent: sessions, nested subagents, turn boundaries, approval waits, token accounting and context-window usage (#1960, #1961)
- `of validate` now fails a recorded onboarding cell whose driver pillar still claims a gap, instead of silently under-counting the driver column (#1968, #1975)
- A new onboarding cell's recipe timing is diffed against its sibling recipes, so an outlier is visible when it is written rather than after a recording run (#1974)

**Fixed**
- The relay link recovers after the Mac wakes from standby. A half-open WebSocket used to leave relay rows frozen behind a green dot, with no banner and no fault, until you toggled the Sources setting (#1953, #1957)
- Every rendered project group is reorderable, including a project served by a second daemon. The last local group's down-chevron no longer renders enabled and does nothing (#1948, #1949)
- Project group order is remembered as a superset, so a project that leaves one refresh keeps its slot instead of being appended last when it returns (#1954, #1956)
- Claude Code sessions no longer print an Irrlicht marker comment into the assistant text you read in the terminal (#1944, #1947)
- A `known_failing` onboarding cell can promote its own sub-100% recording again, without a manual trim-and-restore (#1967, #1973)
- The corpus lock stopped reporting failure on its own success (#1971)
- The Homebrew cask no longer prints a deprecation warning on every `brew` invocation that loads it (#1966)

**Changed / Docs / Distribution**
- Building from source now needs **Go 1.26** — the module's `go` directive moved with a dependency bump while four prerequisite lists still said 1.25 (#1952)
- Replay complexity triage: four findings acted on, eight left in place with cited reasons (#1970, #1972)
- Dev dependency bumps: vitest 5.0.0 in `platforms/web`, plus the viewer tree's own vitest (#1950, #1951)
- Muse was added to every documentation surface that enumerates agents — README's stage table and prose, the site's supported-agent list, the adapter and session-detection references, the state-machine and API adapter enums, quickstart, story and CLI tools

### Technical appendix


- **Menu bar slot budget, grouping, and per-subscription quota slots (#1955, PRs #1958 and #1959).** `MenuBarGrouping` (`project` | `location` | `combined`) and `MenuBarMaxProjects` (1–10, default 5) replace the `menuBarCompact` Bool; compact IS `combined` grouping with a one-slot budget, so it became a point in the new space rather than a control beside it. Density is derived — `aggregatesSessionDots` and `usesNarrowQuotaBars`'s `.usage` arm both ask the grouping — which keeps narrow quota bars reachable on the Usage style. The budget applies to the BUILT group list, after `orderedProjectGroups` skips misses, which is what keeps the icon correct once `projectGroupOrder` became a remembered superset. `MenuBarStatusRenderer.maxGroups` is clamped at the renderer as well as at construction: removing the clamp and running `MenuBarStatusRendererTests` showed a budget of `0` renders every project away and leaves a lone `…` as the whole icon, and a negative budget traps outright in `prefix(budget)`. Phase 2 added `buildLocationStatusSVG` as a second entry point that takes no `projectGroupOrder` — that absence is the design, since a daemon name written into a never-pruned superset would pad the `count - 1` reorder bound forever. The daemon names surface through the icon's `accessibilityDescription`; review found that plumbing was dead in the shipped app, because an explicit `setAccessibilityLabel` on an `NSStatusBarButton` overrides its image's description, so `applyIcon` now re-applies the label on every repaint. Phase 3 replaced the single `menuBarQuotaProvider` with an ordered, comma-delimited `menuBarQuotaProviders`, one slot per entry; an empty list keeps the previous Auto behaviour. Both migrations are non-destructive and guarded on whether the new key has been written. The one defect this change shipped into its own UI — a slot-budget stepper whose number was invisible, because the value was the `Stepper`'s label and the call site carried `.labelsHidden()` — was caught in review and is pinned by `MenuBarSlotBudgetStepperTests`, the only test here seen RED before its fix existed (20.0pt broken versus 33.0/39.0pt fixed, measured through `PinnedSnapshotHost`).
- **Muse adapter (#1960, PR #1961).** Watches `session.jsonl` under `~/.local/share/muse/sessions/<YYYY>/<MM>/<DD>/<session-id>/`. The filename is a constant, so the session ID comes from the parent directory; subagents write their own `session.jsonl` under `subagent/<child-id>/`, so parent linkage falls out of directory nesting with no file read. PID binding reads each session's `.session.lock` stamp, and process discovery matches the `muse-bin-` command-line prefix because the OS process name embeds version and build (`muse-bin-1.2.1-R2847.1`) and the `muse` launcher `exec`s that pinned binary before a session begins. Records are `payload_type`-discriminated envelopes; some lines are instead a `retained_frame` batching several independently re-encoded inner records, and each child is routed through the same dispatcher a top-level line uses, then merged into the single verdict the one-event-per-line parser contract requires. A `kind:"run"` terminal event is the explicit turn boundary and a `kind:"approval"` request routes to `waiting`. Token usage is read from `model_completed` only — `goal_usage_attribution` re-emits the same usage with ownership metadata and would double-count. The context-window limit comes from muse's cached `model-catalog/` files, because the transcript never carries it, and the `session-index.db` sidecar is deliberately never read after it measured out of sync with the live session set. Onboarding stands at 41/50 cells observed and 12/12 core.
- **Relay link recovery after standby (#1953, PR #1957).** A standby/wake cycle can leave a WebSocket half-open — the peer is gone, but no FIN and no error arrive, so `receive()` waits forever and the backoff reconnect behind it never runs. The relay was the visible casualty because the local link self-heals through `+Hydration.swift`'s 30s `projectCostsTimer` REST refetch, while `relaySessionMap` is fed only by the relay socket. It was silent because `relayConnect()` set `.connected` one line after `resume()`, before any frame, so `aggregateConnectionState` stayed `.connected` and `DaemonHealth.faults` returned `[]`. `ConnectionLiveness.isStale(lastFrameAt:now:deadline:)` is a pure static predicate testable without a socket; both inbound handlers stamp a last-frame time before decoding, and a pong stamps and confirms too, since an idle link produces no other evidence and a relay URL may point at a bare `irrlichd` that sends nothing when it tracks no sessions. One repeating tick pings every live link and cancels a socket past the deadline so the existing reconnect machinery runs. Ping 20s and stale 60s straddle `irrlichtrelay`'s own 30s/45s ping/pong, so the client never declares death before the server would. `handleSystemDidWake()` does the same immediately rather than waiting out the deadline, and `.connected` now means the same on both links: a frame or a pong arrived.
- **Reorderable project groups, including relay rows (#1948, PR #1949).** The reorder chevrons took their index and disabled-bounds from `apiGroups`, while the handlers acted on `projectGroupOrder`; `orderedGroups` applied only to local groups and `recomposeApiGroups` appended relay groups after it, so a relay row padded the count the view measured against. `recomposeApiGroups` now orders the whole rendered list, local and relay together, and `reorderMoves(for:)` is the single question the chevrons ask. Two hardenings alongside it: `SessionManager.init` reads the persisted order synchronously rather than inside its deferred `Task`, closing a window where a hydration meeting an empty order would save the daemon's order over the user's; and duplicate names are dropped where they can enter, with the sort index built by hand rather than with `Dictionary(uniqueKeysWithValues:)`, which traps on a repeated key.
- **Remembered project group order (#1954, PR #1956).** `orderedGroups` pruned `projectGroupOrder` down to the names in each payload, so a project that left one recompose lost its slot; the worst case was a recompose before the first hydration pruning everything and saving `[]` over the whole order. The array is now a superset that only ever grows by appending a first-seen name, an empty payload writes nothing, and the rendered reorderable domain is that array narrowed to what `apiGroups` shows. Same UserDefaults key and `[String]` shape, so no migration.
- **No Irrlicht marker in rendered assistant text (#1944, PR #1947).** The premise the question marker rode on — that Claude Code hides a bare HTML comment in assistant text the way it strips one from CLAUDE.md at injection time — was probed in a real TUI (claude 2.1.263) and is false: the comment rendered both inline and after a blank line. The change installs one invariant: no Irrlicht marker may be instructed into text the user reads, and the only carrier a managed block may name is a tool input the surface does not render as prose. `managedTaskQuestionBlock` is retired — its sentinel pair moved to `retiredInstructionBlocks`, so the next granted daemon start uninstalls it from `~/.claude/CLAUDE.md`. No parser changed, so a live session on a pre-fix CLAUDE.md and every frozen replaydata transcript parse as before. The guard and its fixtures live in their own `instructionmarkercarrier_test.go`, because they test a property of the block inventory rather than the file-patching machinery.
- **Onboarding gates (#1967, #1968; PRs #1973 and #1975).** `atomic-promote.sh` refused every sub-100% candidate, including a cell whose own `expected.jsonl` declares `known_failing:true`, which contradicted the documented behaviour and forced a manual trim/promote/restore hit six or more times across the muse sweep. Separately, `matrix.ValidateDriverRecordedConsistency` now fails `of validate` when a cell is `recorded:true` while its `driver_capability` still says `gap:<primitive>` on either the metadata or the `details.assessment` tier — a provably stale state, since a recording proves the driver drove that scenario. The check is scoped to the driver axis only: a `known_failing` cell may legitimately be recorded while `daemon_capability` is `bug` or `incapable`. QA then found the first cut returned exit 0 for a recorded cell whose `details.assessment` omits `driver_capability` or sets it to JSON null, because both decode to Go's zero value; the field is now decoded as `*string`, so absent-or-null is its own loud finding. That inversion is the rule the check exists to uphold — a validator that cannot parse its input checks MORE, never less.
- **Release tooling note.** `tools/security-scan.sh` reported `govulncheck: loading packages` failures across every module during this release. The cause was a locally installed `govulncheck` built with Go 1.25 against a tree whose `go` directive is now 1.26, not a vulnerability; reinstalling the scanner cleared it and the gate passed.
...
Epic #1355 set out to make hooks the tier-1 state channel wherever an agent has one, and to make adding a new adapter a declaration rather than a rewrite. Both halves are done. The safety half — permission-gated installs, mandatory transcript-path confinement, per-adapter version floors, a liveness watchdog, wall-clock ceilings on hook-asserted holds, entry re-verification, registry-driven uninstall, consent folded into the decode path so a receiver cannot forget it — shipped first and gated everything after it. The rollout half then closed adapter by adapter: eight more CLIs now push state instead of irrlicht guessing it from transcript prose, across four different delivery shapes (JSON config splice, comment-preserving TOML, an installed plugin/extension, and a shared shell-store). `aider` was audited and deliberately left unchanged — its notifier signal doesn't fire on ~30% of real turns and never in headless mode, so it can't replace the existing heuristic; that's a recorded negative result, not a gap. `gemini-cli` is scheduled for deprecation in v0.7 as no longer independently verifiable, so its hooks are the tier it ships with, not the start of further investment.

### Also in this release

**Added**
- **kiro-cli** now uses hook-based state detection — `postToolUse` and `stop` events, beacon delivery, a flat entry shape (no matcher-group wrapper) — the cheapest real test of whether the hook machinery generalizes past the first three adapters. (#1716, #1732)
- **gemini-cli** now uses hook-based state detection (`Notification`/`AfterTool`/`AfterAgent`, beacon delivery) — the strongest pilot on the evidence, since upstream ships its own Claude-Code-to-Gemini hook migration command. (#1717, #1724)
- **mistral-vibe** now uses hook-based turn-done detection (`post_agent_turn`) — the first non-JSON hook format, needing a new comment-preserving TOML writer (`hooktoml`) so installing a hook doesn't destroy the user's config layout. (#1718, #1733, #1734)
- **opencode** now observes turn state through a plugin irrlicht installs, since opencode removed its config-file hook path upstream between versions — a shipped-artifact route rather than a config write. (#1719, #1763)
- **pi** now observes turn and tool events (`turn_start`/`turn_end`, `agent_settled`, `tool_execution_*`) through its own first-class extension system — the epic's earlier "permanent tier-5, no approval prompt exists" verdict had stopped one layer short of this. (#1721, #1758)
- **hermes** now observes an approval state its own shared SQLite store cannot express, through its own shell hooks. (#1722, #1765)
- **antigravity** now observes turn end through a hook merged into a config file irrlicht does not own — the shape most different from every prior adapter's config-splice or installed-artifact route. (#1723, #1775)
- **GitHub Copilot CLI** now has a hook-authoritative live-state tier via its native http hooks, on top of the transcript-based detection v0.5.10 shipped. (#1378, #1436)
- **kiro-cli's copied default agent now refreshes on CLI version change** — the grant-time snapshot no longer silently drifts from `kiro_default` across upgrades. (#1736, #1749)
- **`irrlichd` gained a hook-post beacon** that reads the daemon's port from its own address file at fire time instead of a port baked in at install — the delivery mechanism command-only agents (gemini-cli, mistral-vibe) need, and structurally immune to the #1178 stale-port class of bug. (#1373, #1412)
- **Installed hook entries are now re-verified and repaired behind consent**, closing the gap where an agent's own settings UI could silently delete or mangle what irrlicht installed. (#1372, #1422)
- **A permission that is granted but not actually in force is now visible without opening Settings** — a status strip (macOS) and banner (dashboard) reading "N permissions are granted but not applied", with every cause named. (#1385)
- **Click-to-focus works for sessions running in a [herdr](https://herdr.dev/) pane.** (#1350)
- **`of status --summary` and `of coverage --hooks`** give a one-line onboarding-maturity read per adapter instead of hand-inspecting `replaydata/`. (#1374)
- **A "core-12" scenario tier, alpha maturity level, and derived-unobservable cell state** for the onboarding matrix. (#1414)
- **`tools/mutate.sh`**, a shared mutation-testing harness — three separate issues had each independently reinvented the same anchor-count-and-replace shape before it existed as one committed tool. (#1750)
- **Model alias sync**: 4 new frontend model IDs synced from codeburn (`codex-auto-review`, `mimo-v2.5-pro`, `mimo-v2.5`, `GLM-5.3`/`glm-5.3`) — sessions on those models price correctly instead of at $0.

**Fixed** — hook-channel safety hardening
- Hook-asserted persistent holds now have a wall-clock ceiling, so a missed release can no longer pin a session at `waiting` forever. (#1360, #1376)
- A per-adapter hook-liveness watchdog now detects "granted, installed, but nothing arriving" instead of the daemon staying silently blind. (#1368, #1413)
- An unrecognized hook event is now counted and reported instead of an `Info` log nobody reads. (#1364, #1403)
- Every hook adapter now declares a minimum upstream CLI version, refusing to install hooks a too-old CLI can't parse safely. (#1365, #1393)
- Every hook receiver now confines a caller-supplied `transcript_path`, made structurally unskippable via `DecodeConfined` plus an architecture rule rather than a convention someone could forget. (#1361, #1380, #1465)
- Consent is now folded into the decode chokepoint itself, so a hook receiver cannot dispatch without naming the permission that gates it. (#1488)
- `AssertPermissionGated` is now key-aware — a receiver gated on the wrong permission no longer passes the same contract test a correctly-gated one does. (#1475, #1489)
- A failed consent-effect apply is now surfaced and retried instead of silently recorded as "granted" while nothing actually happened. (#1362, #1379)
- A `grant-all` dev/test daemon can no longer rewrite your real agent configs outside its own `IRRLICHT_HOME`. (#1449, #1460)
- `hookjson` installs now preserve comments, key order, and `&&` in command strings — a read-modify-write previously reordered a hand-organized config and HTML-escaped shell operators on first install. (#1371, #1381)
- `--uninstall-hooks`, the recording rig, and install/uninstall now drive their adapter lists off the agent registry instead of a hardcoded two-adapter list guaranteed to miss adapter #3. (#1357, #1375)
- `site/install.sh --uninstall` and `irrlichd --uninstall-task-eta` now actually remove hook entries and CLAUDE.md blocks and signal a running daemon immediately, instead of leaving stale entries that silently reappear on next start. (#1416, #1421, #1425, #1437, #1458)
- claudecode's disclosed hook-consent count is now derived from `installedHookEvents` rather than hand-typed, so it cannot drift from what's actually installed again. (#1356, #1370, #1409)
- A mistyped `irrlichd` flag (e.g. `--recrod`) now fails loudly instead of silently starting a daemon with recording off. (#1417)
- A codex hook-receiver bug and three separate hook-recording blockers are fixed, unblocking real hook-bearing recordings for that adapter. (#1443)

**Fixed** — other
- The macOS `.popover` now reparents as a child window of its host panel, fixing a class of focus/ordering bugs. (#1745)
- Ghostty tab selection now matches by working directory instead of guessing the frontmost tab. (#1470)
- Session detection no longer births a session `working` on metrics that observed nothing. (#1464)
- Every `SessionDetector` and `PermissionService` construction now routes through a single allocator, closing two classes of construction-order bugs. (#1444, #1459)
- Consent is reloaded from the store when another process changes it, instead of a stale in-memory read winning. (#1429)
- Mistral-vibe's hook install no longer fails against a config the CLI itself wrote — `hooktoml` now models multi-line TOML arrays. (#1753, #1755)

**Changed / Docs / Distribution / Security**
- `AGENTS.md` split into a lean index plus topic-scoped `docs/*.md` files, keeping the force-injected root file under its own 400-line budget. (#1744)
- `gemini-cli` is scheduled for deprecation in v0.7 — the adapter can no longer be independently verified on this account tier. (#1725)
- CI badge publishing (ARS score, star-history chart) moved to gist-backed jobs instead of pushes `main`'s branch-protection rule can never accept. (#1654, #1602)
- Dependencies: `golang.org/x/tools`, `modernc.org/sqlite` (twice), `actions/upload-artifact` bumped; no open Critical/High Dependabot or CodeQL alerts at release time.
- A long tail of CI-gate, replay-fixture, and macOS Swift-test reliability hardening landed this cycle without a user-visible change: bounded git/tmux/herdr shellouts, deterministic snapshot fixtures (locale, timezone, scale, `@AppStorage`), hardened workflow badge/gist publishing, and the mutation/registry-tripwire test infrastructure (`tools/mutate.sh`, real-config-fixture registry, adapter-registry-derived completeness checks) that the adapter rollout above leaned on. Full list: #1130–#1712 touch this area; see `docs/ci-gates.md`, `docs/testing-contracts.md`, and `docs/swift-testing.md` for the standing write-ups.

### Technical appendix

- **`core/pkg/atomicfile`** replaces 11 independent hand-rolled `atomicWriteFile`/`AtomicWriteFile` copies (9 adapters plus 2 exported package functions) with one implementation, migrated to the strictest behavior found across all 11 (`os.CreateTemp` over a fixed temp name, 0o600/0o700 modes). (#1761)
- **A shared single-file-artifact installer** (`core/adapters/inbound/agents/hookartifact`) replaces pi's and opencode's ~250-line near-duplicate installers, carrying both adapters' existing mutation tests forward as locks, including opencode's foreign-file-clobber-on-uninstall protection. (#1764)
- **Six adapters' hand-written hook-version-gate tests** were deleted where the shared `contracttesting.AssertHookVersionGate` contract strictly dominates them, proven per-adapter by a floor-weakening mutation rather than by inspection. (#1762)
- **A registry tripwire** now requires every hooks-installing adapter to declare a real captured config fixture, closing the blind spot where every adapter graded its own install against a config its own tests wrote — the exact gap that let mistral-vibe's #1753 regression through undetected. (#1756)
- **`hookcov`** now attributes `hook_received` events by cross-referencing sibling events for the same session, fixing miscounted coverage for adapters whose turn-done hook name collides with another adapter's canonical wire name. (#1768)
- **The recording rig** gained a promote-time hook-free check, a spawn-path `unapplied_grants` poll (closing a race where the daemon's socket accepts connections before its permission-apply pass completes), and corrected coexist-mode adapter docs. (#1754)
- **The rig's agent-home Source-variant tripwire** is now existence-checked via an AST scan of `core/domain/agent`, instead of fataling only when some future adapter happens to exercise an unhandled variant. (#1767)
- **kiro-cli's own SQLite store** is now declared as an `Advisory` write — a third permission-declaration category for paths an adapter's `Apply` writes to that irrlicht doesn't own and can't safely back up/restore. (#1748)

Found in #1355. Prior related work: #1129 (superseded), #1719–#1723 (per-adapter rollout issues, closed).
...
Three things grew this cycle. The adapter roster went from nine to eleven: **GitHub Copilot CLI** and **Hermes Agent** are both monitored now, and neither was a copy of an existing adapter — Copilot writes explicit turn boundaries into a uniformly-enveloped `events.jsonl`, and Hermes writes no transcript files at all, keeping every session in one shared SQLite store. **herdr** joins tmux and kitty as a terminal backend you can type into, interrupt, and read back from the daemon, headless and over the relay. And the state machine stopped guessing: Claude Code's `Stop` and `Notification/idle_prompt` hooks and Codex's `PermissionRequest`/`Stop` hooks are now authoritative live signals, so "an approval prompt is on screen" and "the turn is over" are observed facts rather than inferences from the tail of an assistant message.

Underneath that, the signal machinery got a shared spine — explicit tiers, one ordered policy table, a clock, and an arming predicate — so a signal now declares how far it can be trusted in one place instead of each classifier rule restating it. Four separate `fswatcher` fixes close the class of bug where a session was simply never observed and nothing said so: a dropped kqueue creation event is now caught by a periodic reconcile sweep, a watch registration that fails mid-scan is reported instead of swallowed, and the reporting is capped so an FD-exhaustion storm doesn't rotate its own first lines out of the log.

The rest is correctness and hygiene: a slow-but-executing edit is no longer mislabelled as a stalled prompt, a timed-out `lsof` probe no longer permanently kills a live background process, the macOS menu-bar panel stopped burning ~10% CPU while closed, and the master "Enable notifications" toggle actually silences everything including the spoken alerts. Security-wise, 4 CodeQL alerts and 89 SonarQube findings were cleared, `golang.org/x/net` was bumped twice, and every ecosystem in the repo now gets scheduled dependency updates rather than waiting for an alert.

### Also in this release

**Added**
- **GitHub Copilot CLI is now a monitored agent** — the tenth adapter. Sessions are discovered from `~/.copilot/session-state/<session-id>/events.jsonl` (relocatable via `$COPILOT_HOME`), with the session id taken from the containing directory rather than the filename, which is constant. Copilot writes explicit turn boundaries, so `working`/`ready` ride on real records instead of a quiet-tail heuristic; permission prompts route to `waiting` from the transcript itself, with no hook installation required; and tokens and cost are live per assistant message rather than flat-zero until the agent exits. In-process subagents (`task`/`explore`) are counted on the parent, which is where Copilot records them — it creates no child session directories. Verified against CLI 1.0.77–1.0.78. (#1256, #1332)
- **Hermes Agent is now a monitored agent** — the eleventh adapter, and the first one onboarded whose sessions have no files of their own: every session lives in one shared SQLite store at `$HERMES_HOME/state.db`. A row appears while the session is still live, and `finish_reason` is an explicit turn boundary, so no idle-flush heuristic is needed. Verified against Hermes Agent v0.19.0 on macOS. (#1325, #1327)
- **[herdr](https://herdr.dev/) is now a supported terminal backend**, alongside tmux and kitty. A session running in a herdr pane can be typed into, interrupted and read back from the daemon — headless, with no macOS automation prompt, and reachable over the relay — by scripting `herdr pane send-text` / `send-keys` / `read` against the pane recorded in `$HERDR_PANE_ID`. Verified end to end against herdr 0.8.0 driving a real Claude Code TUI. (#1348, #1349)
- **Codex has a hook-authoritative live-state tier.** The daemon installs `PermissionRequest`, `PostToolUse` and `Stop` hooks into `~/.codex/hooks.json`, so an approval overlay flips the session to `waiting` the moment it appears rather than when prose matching happens to notice it, and turn-end arrives as a push instead of an inference. (#1171, #1174)
- **Claude Code's `Stop` hook is now the authoritative `ready` signal**, replacing the transcript-tail `IsAgentDone` heuristic and its codex carve-out at true turn end. (#1161, #1170)
- **Claude Code's `Notification/idle_prompt` hook now routes to `waiting`** — the false-negative the prose heuristic structurally cannot reach: a turn that ends idle with no trailing question or cue, which used to show `ready`. (#1173, #1175)
- **Signals now declare an explicit tier and reconcile through one shared mechanism**, so a hook-fed fact and a transcript-derived guess are no longer weighed by ad-hoc precedence scattered across classifier rules. (#1288, #1295)
- **A waiting session's headline says what the question is about.** Every pending question is now shaped `<3–5 word topic>: <question>` — "DB migration: drops users.legacy_id — run it or keep the column?" instead of a context-free "Should I proceed?". (#1142, #1146, #1186, #1193)
- **A Tips & Tricks page on the docs site**, collecting the easy-to-miss ways to get more out of Irrlicht — starting with keeping the menu bar always visible, which was previously buried inside macOS Setup. (#1317)
- **`.claude/skills/**/*.md` is linted as its own preflight and CI gate.** Skill files decide how agents triage, plan, implement and review, and until now nothing mechanical read them. (#1209, #1280, #1290)
- **`/ir:triage` emits an effective complexity level 1–10 plus the run plan `/ir:exec` should spend**, moving the budget decision to where the reading has already happened. (#1204)
- **A red-first test gate and an evidence bar for dismissals.** A test asserting a defect must be seen failing before the fix exists, and any claim whose function is "you don't need to look here" now carries the same evidence bar as the claim it supports. (#1126, #1201)
- **`/ir:code-review` is a repo-owned, agent-invocable reviewer.** `ir:exec`'s review gate pointed at the built-in `/code-review`, which is `disable-model-invocation` and unreachable — so every run silently degraded into an improvised pass. (#1205, #1217)

**Fixed**
- A session is no longer invisible for the daemon's whole lifetime because macOS dropped a file-creation event. fsnotify's kqueue backend discovers new files by diffing a directory listing, so a creation can be missed outright; a periodic reconcile sweep now re-walks the tree and reports what fsnotify didn't. (#1248, #1286)
- A watch registration that fails during the tree scan — FD exhaustion, a permissions change, Linux's inotify limit — is now reported instead of discarded. The watcher previously signalled "ready" over a subtree it was not watching, and every session under it was simply never observed, with nothing logged. (#1255, #1279)
- Watch-failure reporting is capped and skips vanished directories, so an EMFILE storm reports the first failures individually plus one summary total rather than tens of thousands of lines that rotate the informative ones out of the log. (#1289)
- A timed-out `lsof` probe is no longer read as "this background process is dead". The caller responds to a dead verdict by purging the process and persisting that to the ledger, so one slow probe could permanently drop a still-running background process and flip a live session to `ready` with no way back — and `lsof` is slowest exactly when the machine is busiest. (#1307)
- A slow-but-executing edit is no longer flagged as stalled. The threshold was the 5s poll cadence, which sits inside the observed duration tail for legitimately-running edits, so a 14–16s edit flickered `working` → `waiting` → `working`. It is now a named 30s threshold set past that tail; a genuinely held prompt still trips it. (#1130, #1132)
- The macOS menu-bar panel no longer burns ~10% CPU while closed. `orderOut()` left the SwiftUI session list window-attached, so the working-dot animation and every coalesced WebSocket flush kept re-laying-out the full list off screen. (#690, #1134)
- The macOS master "Enable notifications" toggle is authoritative. It was wired only to the Settings UI, so turning it off collapsed the section but left the per-event keys `true` behind it — banners kept posting and the Speak-aloud voice kept talking. (#1183, #1202)
- A waiting cue or question sitting before the trailing 200 runes of a long final message now flips the session to `waiting`. The prose heuristics only ever saw the display-truncated tail. Fixed for the shared path and then for Codex, which never set the flag at all. (#1150, #1155, #1159, #1163)
- A turn that ends with an `irrlicht-question` marker routes to `waiting`. The marker fed only the display headline, so the headline showed a pending question while the state contradicted it. (#1138, #1145)
- `irrlicht-focus` resolves the daemon's port instead of hardcoding `:7837`, so focusing a session monitored by a dev daemon on an alternate port no longer sends the request to production. (#1215, #1241)
- The agent hooks the daemon installs now POST to the port the daemon actually binds. A daemon on an alternate port installed hooks that delivered to whatever owned `:7837`, so its own hook-authoritative tier silently received nothing; existing `:7837` entries are recognized as ours and repointed in place rather than orphaned beside a duplicate. (#1178, #1207)
- The task ETA converges as a task progresses instead of staying anchored to one unrepresentative round. Measured over 398 transcripts / 116 episodes: MAE 241.6m → 195.5m, median absolute error 25.9m → 21.7m, jitter 2.4m → 59s. (#977, #1211)
- Replay pairs each recording with its sibling `events.jsonl` sidecar. The resolver only probed a legacy filename, so `os.Stat` always failed and all ~380 recordings replayed transcript-only — the sidecar path had no fixture coverage at all, which is how the hook→signal drift in #1320 could sit undetected and why "no goldens moved" was vacuous evidence on the PR that fixed it. (#1341)
- The hook-name → `SignalKind` mapping is one shared table. It was written twice, and the two had drifted: the daemon held a permission signal on `PreToolUse`, the replay harness fell through and returned. The paths agreed by luck. (#1320, #1323)
- `npm test` self-heals in a fresh worktree. `node_modules/` is gitignored, so a new `git worktree add` died with `vitest: command not found` — which reads as "no web tests here" rather than "deps missing". A `pretest` script now runs `npm ci --ignore-scripts` only when dependencies are absent, never rewriting the lockfile. (#1229)
- The nightly star-history chart builds again. GitHub now blocks the REST stargazers listing for personal access tokens; the fetch is ported to the GraphQL stargazers connection. (#1187)
- 11 SonarQube security findings resolved, including a BLOCKER path injection where Codex's hook receiver passed `transcript_path` straight from an HTTP body to `os.Open`. The path is now confined to a `.jsonl` rollout resolving inside `$CODEX_HOME/sessions`, with symlinks resolved before the containment check. (#1196)
- The 4 open CodeQL code-scanning alerts are closed — three workflows without a `permissions:` block, and one `go/path-injection` sink that now carries a local guard CodeQL recognizes. (#1259, #1260)
- 78 of the 80 open SonarQube findings cleared across the shell, Go and web analyzers, plus the earlier shell-rule sweep in `tools/lib/`. (#1250, #1262)
- `golang.org/x/net` bumped to v0.56.0 (GO-2026-5942 / CVE-2026-46600, reachable in called code and therefore failing every pre-push hook for a diff touching `core/`), and the `go` directive floor raised to 1.25.12 so Go's toolchain switching clears 38 stdlib advisories per module automatically. (#1247, #1252, #1253, #1296)
- `tools/security-scan.sh` gained `--changed` scoping, so a pure-Go push no longer pays for an `npm audit` of both web trees and gets rejected by a pre-existing advisory it could not have caused — the failure mode that forces `--no-verify`, which disables every other gate too. (#1213, #1231, #1242)
- A network-failed `npm audit` is no longer reported as a fabricated advisory. npm exits non-zero both when it finds advisories and when the audit cannot run; the report's shape now decides which. (#1140)
- The pre-push preflight scopes every gate to the packages and web trees the push actually touches, so a typical push finishes in seconds instead of blowing past an automated caller's command timeout and being killed after the commit had already landed. (#1151, #1154)
- The security scanner derives its module list from `go.work` instead of a hardcoded copy, so a seventh module cannot be added and silently never scanned. (#1293)
- The roundtrip testbed image installs `@anthropic-ai/claude-code` with `--ignore-scripts` and then runs only the one named install step it actually needs, instead of letting every package in the tree run install hooks. (#1274)
- A session running inside a herdr pane no longer takes on a stranger's terminal identity. herdr's server owns each pane's pty, outlives any attached client and is reparented to init, so every terminal variable a pane inherits — `$TERM_PROGRAM`, `$TMUX`, `$TMUX_PANE`, `$KITTY_*`, `$VSCODE_PID` — describes whatever environment the *server* happened to start in. Irrlicht captured those verbatim, so a click-to-focus jumped to an unrelated application, and a server started from inside tmux made the backchannel resolve to tmux and type into a foreign pane in a different window. (#1348)
- Every newly discovered session is now classified against its own metrics, not just child sessions. A top-level session was birthed flat `ready` and only re-classified when it had a `ParentSessionID`, which is invisible for an agent whose transcript file is created empty and filled in later — but very visible for one that writes the file only when the first prompt is sent, so the open turn is already on disk at discovery. Such a session reported idle while the agent was demonstrably generating. Found while onboarding GitHub Copilot; the fix is adapter-agnostic. (#1256)
- `MergeMetrics` now carries `TranscriptPermissionPending`, so a transcript-tier permission prompt actually routes the session to `waiting` on the live path. The merge is an explicit field allowlist and the field was missing from it, so the flag was computed correctly, survived replay through a separate converter, and was then dropped on the only path that matters. (#1256)
- `exit_plan_mode` and `ask_user` are now recognized as user-blocking tools. Both hold a persisted tool call open while the user decides, so without an entry the session sat in `working` while it was really waiting. (#1256)
- Open subagents are now counted from the parser that actually read the transcript. `agents.SubagentCounters` built one parser at wiring time and closed over it, while the metrics adapter builds a separate parser per transcript path — so any adapter keeping its count in parser state reported zero forever. (#1256)
- `nanoid` bumped to 3.3.18 in both web trees, clearing [GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8) (high, custom generators can loop indefinitely when size is zero). Dev-only — it reaches the trees as a transitive dependency of the vitest toolchain and ships in no artifact — but it failed this release's own security gate, and the lockfiles were bumped with a newer npm than the local default so the `libc` fields on the `@rolldown/binding-linux-*` entries survived the write.
- Model alias map re-synced against codeburn upstream: 4 new aliases added (`quickdesk-auto`, `k2d6-agent`, `k3`, `k3-agent`).

**Changed / Docs / Distribution**
- The release playbook tags the exact squash-merge commit of the release PR rather than `HEAD`, and asserts the tagged tree's `version.json` before pushing — v0.5.9 nearly shipped its tag on a star-history bump. (#1135, #1139)
- The Claude Code and Codex hook installers share one `hookjson` installer and one waiting-cue rule, instead of two adapters maintaining a near-verbatim copy of the same logic. (#1179, #1206)
- "The installed hook endpoint follows the daemon's bind address" is now a shared contract assertion (`contracttesting.AssertHookEndpointFollowsBindAddr`) that each hook-installing adapter wires with one call, so a future adapter cannot quietly ship a hardcoded port. (#1216, #1240)
- `signalPolicies` gained a clock and an arming predicate, `PreCompact` and the stalled-edit-tool overlay moved onto the shared `SignalHolds` mechanism, and the policy map plus order slice folded into one ordered table — so a policy that is added but not ordered is now unrepresentable rather than test-caught. (#1306, #1318, #1324)
- The onboarding factory has one owner for the record-daemon lifecycle, and its guards prove *freshness* rather than presence — seventeen findings from the Copilot and Hermes runs shared the shape "confirms something exists where it means it came from this run", three of which composed into a fully-green path to committing another run's data. (#1214, #1239, #1333, #1337)
- `ir:exec` gained a commit step, a stale-playbook check, a duplicate-work guard, base re-fetch before opening the PR, WIP markers while a run is still pushing, needs-info surfacing in every mode, and review depth scaled to the diff. (#1153, #1160, #1162, #1220, #1221, #1225, #1227, #1277, #1278, #1287, #1328)
- Both web trees moved to Node 24 + jsdom 30, and dependency updates are now scheduled weekly for every ecosystem in the repo (Go modules, both npm trees, GitHub Actions) rather than arriving only when a security alert fires. (#1225, #1253, #1304)
- `AGENTS.md` gained a Process Rules heading and improvement-suggestion guidance. (#1136)
- `/ir:release` follows `/ir:refresh-aliases` by path rather than by name, so a skill added or renamed after a session started is still reachable. (#1249, #1254)

### Technical appendix

- **GitHub Copilot CLI adapter (#1256, #1332).** Copilot writes an append-only, uniformly-enveloped `events.jsonl` per session under `$COPILOT_HOME/session-state/<session-id>/`, so a plain `JSONLineParser` covers it and no sidecar read is needed; the session id comes from the directory via the existing `FilesUnderRoot.SessionIDFromPath` seam. All three states are transcript-derivable, which is what removed the planned hook tier from scope. Reaching `waiting` required one shared addition: the classifier's permission rule read a hook-fed field no parser can set and that is never populated under replay, so this adds the transcript-tier twin (`ParsedEvent.PermissionRequest`/`Resolved` → `MergeMetrics`). Three adapter-agnostic bugs fell out of the onboarding: `MergeMetrics`'s explicit field allowlist was missing `TranscriptPermissionPending`, so the flag was computed correctly, survived replay through a separate converter, and was dropped on the only path that matters — parser tests, classifier tests and every replay golden were green against completely broken production behaviour; every newly discovered session is now classified against its own metrics rather than only child sessions, which is what birthed a top-level session flat `ready` while the agent was demonstrably generating; and `agents.SubagentCounters` built one parser at wiring time and closed over it while the metrics adapter builds a parser per transcript path, so any adapter keeping its count in parser state reported zero forever (Claude Code was immune by accident — its counter ignores the receiver, and its unit test passed for the same reason). `exit_plan_mode` and `ask_user` are now recognized as user-blocking tools.
- **Hermes Agent adapter (#1325, #1327).** Hermes persists no transcript files — every session lives in one shared SQLite store at `$HERMES_HOME/state.db` — so this generalizes the daemon wiring that had hard-coded opencode as the only `ProcessOwnedStore` adapter, and registers the onboarding-factory column with a working interactive driver. Everything was verified against v0.19.0 on macOS rather than derived from docs: a `sessions` row is INSERTed ~2s after launch with `ended_at` NULL and `message_count` incrementing, so the store is a live discovery source rather than a post-hoc archive; `finish_reason` is an explicit turn boundary (`stop` ends the turn, `tool_calls` means work continues); and `sessions.cwd` is populated only by Hermes' `tui_gateway`, so an interactive `hermes chat` reports source `cli`. The store is opened through `core/pkg/sqlitero`, per the convention that an adapter reading an agent-owned SQLite store never calls `sql.Open` directly.
- **herdr terminal backend (#1348, #1349).** herdr's server owns each pane's pty, is long-lived, detached (PPID 1) and outlives any attached client — so `$TERM_PROGRAM`, `$TMUX`, `$TMUX_PANE`, `$KITTY_*` and `$VSCODE_PID` inside a pane all describe the environment the *server* started in, frozen at that moment and handed to every pane it will ever spawn. Capturing them verbatim sent click-to-focus to an unrelated application and — verified against a server started from inside tmux — made `resolveBackend` pick tmux and type into a foreign pane in a different window, which is worse than reporting the session uncontrollable. Launcher capture now takes a herdr pane's own address (`$HERDR_PANE_ID` plus `$HERDR_SOCKET_PATH`, herdr's documented equivalent of `tmux -S <socket>` and the only key that distinguishes one running server from another) to the exclusion of everything else. Submitting input goes through `pane run` rather than a trailing carriage return, because a CR carried inside `send-text` stops submitting once the text wraps in the agent's composer — so a realistic prompt was typed and left unsent.
- **Hook-authoritative live state for Claude Code and Codex (#1161, #1170, #1173, #1175, #1171, #1174).** Claude Code's `Stop` hook registers with no matcher (Claude Code rejects one) and is wired through the detector as a consume-once overlay carrying `last_assistant_message`, the display-truncated text, and a full-text waiting-cue verdict computed via the same `WaitingScanWindow` the parser uses. `Notification/idle_prompt` ships as a live-only `IdlePromptPending` overlay consumed by a new classifier rule that overrides the `ready` verdict, held while the finished turn stays idle and cleared the moment a new turn begins so it never pins `waiting` into the next turn — the shape the #1141 spike argued for after showing the hook is late-but-authoritative. Codex's tier mirrors it against `~/.codex/hooks.json` (a dedicated file, never `config.toml`, so a malformed write cannot corrupt the user's main config): `PermissionRequest` → live `waiting`, `PostToolUse` clears the pending overlay once an approved tool runs, `Stop` → `ready`.
- **Signal tiers, policy table, clock and arming predicate (#1288, #1295, #1306, #1318, #1324).** `SignalHolds.Hold` records arrival time and takes the clock from the caller (`time.Now` in the daemon, virtual time in the replay harness) so staleness stays testable and replay-deterministic; `stale` widened to `func(HoldContext) bool` carrying metrics, held-since and now, which is what let `PreCompact` (#657) stop being hand-rolled — its rule is half wall-clock, and the old `func(*SessionMetrics) bool` could ask "do the metrics contradict me" but never "have I been held too long". `signalPolicy` then gained a symmetric `ripe func(holdContext) bool` (nil = ripe on arrival) evaluated between stale and apply, so arm-then-fire signals became expressible and `markStalledEditTool` (#488) — the last hand-rolled per-session overlay map on `SessionDetector` — became an ordinary policy row. `stale` is evaluated before `ripe`, so a hold that is both over and not yet ripe is dropped rather than left waiting for a due date that can never arrive. The follow-up fold merged the policy map and order slice into one ordered table, added `session.TierOf` so a signal's tier is declared once instead of restated by every rule that reads it, and stamped provenance off a structural signal rather than comparing reason prose.
- **fswatcher reconcile sweep (#1248, #1286).** fsnotify's macOS kqueue backend discovers new files by diffing a directory listing rather than receiving a per-file notification, so a creation can be missed outright — 660 instrumented runs delivered the linked new-session event every time (worst case 9.9ms), which is why neither a longer deadline nor a sleep would have helped. `Watch` now runs a periodic sweep that re-walks the tree and reports what fsnotify didn't: a transcript with no emission recorded becomes an `EventNewSession`, one whose size moved becomes an `EventActivity`. Every emission goes through a new `emit()` that records the reported size, so in the healthy case the sweep finds nothing to say and stays silent — notifications remain the fast path.
- **fswatcher registration reporting (#1255, #1279, #1289).** `addExistingDirs` discarded the result of every per-subdirectory `watcher.Add`, then called `signalReady()` unconditionally, so `Ready()` reported the tree armed over a subtree it was not watching. Both tree scans now route through one `addWatch` helper that reports a rejected registration via the `Logger`, per Key Conventions' logged-not-propagated rule, and stays deliberately non-fatal — one unreadable directory must not strand the whole watcher, which is why the error was discarded in the first place. The review pass then capped the logging: every failure mode this targets (EMFILE, `fs.inotify.max_user_watches`) is process-global, so once one trips mid-scan every remaining `Add` fails and one line per directory becomes tens of thousands per boot, rotating out the lines that name where the budget ran out. A directory removed between being enumerated and being armed returns ENOENT and is not a blind spot — nothing is left under it to observe — so it is neither counted nor reported.
- **`lsof` timeout (#1307).** `anyLiveOutputWriter` ran `lsof` under a 2s timeout and discarded the error, so a timeout was indistinguishable from "no live writer". The doc comment called that "the safe (don't pin forever) degradation", but the caller responds to a dead verdict with `PurgeDeadBackgroundProcs`, which purges from the tailer *and* persists through `saveLedger` — so the staleness does not self-correct. The correlation runs the wrong way too: `lsof` walks every process on the machine, so it is slowest exactly when the machine is busiest, which is when long-running background work is most likely.
- **Stalled-edit threshold (#1130, #1132).** `markStalledEditTool` set `OpenToolStalled` once a permission-gated edit tool had been open for `staleWorkingRefreshInterval` (5s) and rule 1b routed that straight to `waiting`, but 5s is a *poll cadence*, not a "a human is looking at a prompt" threshold. Edit tools are usually near-instant (median ~0.1s, mean ~1.4s) with a real long tail — legitimately-executing, prompt-free edits of 14–16s have been observed — so the gate sat inside that tail. A named `stalledEditToolThreshold` (30s) sits past it; the #488 hook-unavailable path still trips on a genuinely held prompt.
- **macOS hidden-panel rendering (#690, #1134).** With the panel closed, `orderOut()` kept the `NSHostingView` window-attached, so the whole SwiftUI session list continued animating and laying out off screen: the working-dot `repeatForever` breathe re-rendered at frame rate and every coalesced WebSocket flush (10/s under any working session) re-laid-out the full non-virtualized list. `MenuBarController` now attaches the hosting controller only while the panel is shown, and `SessionManager`'s coalescing window is visibility-tiered — 0.1s visible (unchanged), 2s hidden — so the consumers that matter with the panel closed (menu-bar quota bars, context-pressure alerts, the debug-state file) stay fresh without riding the full render path.
- **Master notification toggle (#1183, #1202).** The gate moved to `SessionManager.sendNotification`, the funnel where both the `UNNotificationRequest` (banner + sound) and the separate `SoundPlayer.speak` call sit — one guard silences every channel and no future caller can route around it, which matters most for the TTS path since it bypasses `UNNotificationContent` entirely. "Key absent" falls back to the per-event OR: an install predating the toggle (#940) that hasn't opened Settings since has no stored value, and silencing those would be a worse regression than the bug.
- **Waiting cues past the display tail (#1150, #1155, #1159, #1163).** `ExtractQuestionSnippet`/`ExtractWaitingCue` only saw `LastAssistantText`, tail-truncated to 200 runes for display. The claudecode parser now scans a bounded window of the *full* assistant text and carries the verdict as a per-pass `PendingWaitingCue`, threaded `ParsedEvent` → tailer state (+ledger, restart-safe) → `SessionMetrics` → `TailerToDomain` → domain metrics, OR'd into `IsWaitingForUserInput`. Display truncation is unchanged, so the sidebar/tooltip contract is preserved. Codex then mirrored it in `parseCodexMessage` with no domain or tailer changes, since the plumbing was already adapter-agnostic.
- **Question marker → waiting (#1138, #1145).** The parsed `irrlicht-question` marker fed only the display headline while the classifier relied solely on `IsWaitingForUserInput()`, whose prose heuristics see only the truncated tail — so on a long final message the headline showed a pending question while the state contradicted it (observed on real session 71f27332). `SessionMetrics.PendingQuestionMarker` is set in the single tailer→domain `Convert` seam and only from a real `TaskQuestion` marker, never the `away_summary` recap or the `LastAssistantText` fallback, both of which also populate `QuestionHeadline`.
- **Topic-prefixed question headlines (#1142, #1146, #1186, #1193).** Two production paths converge on one output shape. The marker path bumps the `irrlicht-question` instruction to v3 asking for `<topic>: <question>` directly, routed through a new `CompactQuestionVerbatim` kind so a multi-sentence marker keeps its topic prefix instead of losing it to sentence selection. The regex path derives a 3–5 word topic from the session's first user prompt (slash-command-stripped, whitespace-collapsed, word-boundary-cut, rune-capped) and joins it to the extracted question, re-capped through the verbatim kind so topic and question share one budget and the *question tail* is trimmed, never the topic. The `~70` char cap relaxed to a soft `~90` — clarity over brevity — which survives to the UI because daemon-side compaction caps at 200 runes and the em-dash/semicolon/colon separators are not sentence terminators. Marker key and sentinels are untouched, so `patchManagedBlock` upgrades installed v1 blocks in place on the next daemon start.
- **Hook endpoint resolution (#1178, #1207, #1215, #1241, #1216, #1240).** `core/pkg/daemonaddr` is the single source of truth for the bind address and the loopback URL same-machine clients should use. Resolution is a pure function of the environment on purpose: the installers run from permission `Apply` closures inside adapter packages constructed before the TCP listener exists, so there is no resolved address to thread down to them. `irrlicht-focus` needed a second source — `ClientURL` prefers an explicit `IRRLICHT_BIND_ADDR`, else the address the running daemon published to `<IRRLICHT_HOME>/irrlichd.addr`, else the default — because a client cannot assume it inherited the daemon's environment: the macOS app plants the variable only in the daemon child's environment and does not launch `irrlicht-focus` at all. The three obligations (an install writes the resolved port; an entry left by a daemon on another port is rewritten in place with no duplicate matcher group; uninstall is not port-scoped) were encoded twice verbatim across two adapters' `hookport_test.go`, so they moved into `contracttesting.AssertHookEndpointFollowsBindAddr` — the shape `core/internal/contracttesting` exists for, with `AssertPermissionGated` as precedent.
- **ETA convergence (#977, #1211, #1223, #1228).** `ForecastTaskCompletion` splits into `ObservedRoundRate` (the measurement plus the observation count backing it) and `BlendRoundRate` (Bayesian shrinkage toward the corpus prior, weighted by that count), so the first measured round nudges the estimate and the fifth largely replaces the prior; #753's zero-round bootstrap is untouched, so a number still appears at the very first marker. Re-measured with `tools/eta-research` over 398 transcripts / 116 episodes against the shipped prior-bootstrap: MAE 241.6m → 195.5m, median absolute error 25.9m → 21.7m, median relative 100% → 96%, jitter 2.4m → 59s. Separately, the age/stale/title preamble that each ETA-chip branch rebuilt by hand (3 branches per frontend, 6 sites, each with a bare `180` literal) collapsed into one `etaChipHeader` helper per frontend, with the threshold named in both and cross-referenced to the daemon's `session.TaskEstimateGraceAge`; the macOS snapshot baselines are byte-identical.
- **Replay sidecar pairing (#1320, #1341).** `resolveInputPaths` only probed the legacy `<transcript-stem>.events.jsonl` spelling, but every sidecar in `replaydata/` is a plain `events.jsonl` beside its transcript — so `os.Stat` always failed, `useSidecar` stayed false catalog-wide, and both fixture drivers replayed all ~380 recordings transcript-only. `applyHookEvent` was therefore never reached over a real recording, which is how the hook→signal mapping could drift without moving a single golden, and why "no goldens moved" was vacuous evidence on the PR that introduced the drift. Probing the sibling name turns sidecar mode on catalog-wide.
- **Shared hook-name → `SignalKind` table (#1320, #1323).** The mapping was a switch in `SessionDetector.HandlePermissionHook` and again in the replay harness's `applyHookEvent`, and the two had drifted: the daemon holds a permission signal on `PermissionRequest` *and* `PreToolUse`, the harness recognized only the former and fell through `default: return`. `replaydata/` carries two `PreToolUse` events, each followed within ~100ms by a `PermissionRequest` producing the same hold — so the paths agreed by luck. The table did not land next to the hook-name constants in the claudecode adapter as proposed, because `core/architecture_test.go` bars `application/services` from importing `adapters/inbound`.
- **Security sweep (#1196, #1247, #1250, #1252, #1253, #1259, #1260, #1262, #1274, #1293, #1296).** The BLOCKER was Codex's hook receiver passing `transcript_path` from an HTTP body straight to `os.Open`, letting any local process able to reach the hook port steer the daemon into reading a file outside the `~/.codex/sessions` tree the `transcripts` permission scopes consent to; `confineToSessionsDir` now requires a `.jsonl` rollout resolving inside `$CODEX_HOME/sessions`, resolves symlinks before the containment check so a link planted inside the tree cannot point out of it, and rebuilds the path from the trusted root so no caller-controlled string reaches the open. CodeQL's four alerts were three workflows missing a `permissions:` block plus one `go/path-injection` whose existing `filepath.Rel` + `HasPrefix` confinement is not a shape the query recognizes as a sanitizer, so it got a sink-local guard as defense-in-depth. On dependencies: nothing in this repo ever moved one except a GitHub security alert, which is alert-driven and one-shot — postcss was patched in one web tree and left stale in the other (#1225), and `x/net` sat on v0.55.0 carrying GO-2026-5942 since its one security bump. Both surfaced by `security-scan.sh` rejecting an unrelated push, which is the worst possible time to find them and the reason a chronically-red gate trains people to reach for `--no-verify`. Dependabot now covers every ecosystem weekly, `security-scan.sh` derives its module list from `go work edit -json` so it cannot disagree with what the toolchain builds (an empty parse hard-fails rather than looping over nothing and reporting a pass), and the `go` directive floor moved to 1.25.12 — a floor rather than a pinned `toolchain` line, because pinning would force one exact toolchain download on every contributor and CI job to fix one machine's staleness.
- **Skill-file linting (#1209, #1280, #1290).** `tools/skill-lint.sh` runs five checks over every `.md` under `.claude/skills/` plus any other tracked `SKILL.md`. Unresolved conflict markers and leftover `{{TOKEN}}` / `REPEAT:` / `OPTIONAL:` scaffolding are hard failures; the internal-reference, list-count and frontmatter checks warn until their noise floor is known (`--strict` promotes them). The checks read a prose view with fenced blocks, inline code spans and frontmatter blanked, because markdown that *documents* a marker is not markdown that *has* one. That blanking is also where the review pass found three silent-pass paths, each reproduced before fixing: an unclosed fence blanked every following line so one missing backtick defeated both ERROR checks and the file reported exit 0 — fence balance is now decided first and an odd count is itself an ERROR; an unterminated frontmatter bound its closing `---` to the first thematic break in the body and swallowed everything between; and frontmatter was blanked for the check that reads `description:`, the field the loader matches on. The rule the fixes encode: when a file cannot be parsed with confidence, degrade toward *more* checking, never less.
...
**Added**
- The History Activity chart is now behind a beta toggle in Advanced Settings, default off, on both the web dashboard and the macOS app. The matrix is reconstructed from opt-in recordings, and a never-recorded bucket renders identically to an idle one — so for anyone not recording it was a grid of blanks reading as "idle". Turning the toggle off while Activity is on screen backs out to the default chart rather than stranding the view. (#1080, #1075)

**Fixed**
- A Claude Code parent session no longer stays stuck at `working` forever once it has spawned a background subagent. Absence of `pendingBackgroundAgentCount` on an end-of-turn `turn_duration` line now normalizes to zero, and the count survives a daemon restart via the ledger instead of going inert. (#1098, #1076)
- Mistral Vibe sessions are found when `$VIBE_HOME` is set. Upstream honors the variable; the adapter hardcoded `~/.vibe` and found nothing. (#1108, #1095)
- Mistral Vibe sessions are found when `[session_logging].save_dir` is set in `config.toml`. Vibe writes that key into every installation's default config on first run as a resolved absolute path, so it is an inviting edit rather than an obscure knob. (#1123, #1115)
- Kiro CLI sessions are found when `KIRO_HOME` is set. Kiro CLI v2.3.0 added the variable; the adapter was authored after the sweep that wired the equivalent fix into `codex`, `claudecode`, and `pi`, and was missed. (#1074)
- Mistral Vibe token counts and cost no longer flatline at zero for the rest of a session after an in-place rewind. The parser's token high-water mark and todo reconciler now reset when the tailer detects a rotation, instead of computing a negative delta against the rewound session's smaller counters and clamping to zero forever. Vibe's auto-compact threshold also now reads the effective per-model value rather than the global default. (#1078, #1063)
- The tailer detects a transcript rewritten in place, not just one that shrank. A rewrite whose new size was greater than or equal to the last read offset previously took the incremental path — seeking to a stale offset inside a file whose earlier bytes had changed, resuming mid-record, and accumulating on top of pre-rewrite state. A fingerprint of the bytes ending at the resume point now catches this, and it is persisted so a rewrite landing while the daemon is down is caught on the next boot. (#1120, #1104)
- Open tool calls are cleared when a transcript rotates, so a call left open in the pre-rotation content can no longer keep a session pinned. (#1118, #1088)
- Mistral Vibe now forces the `waiting` state when it asks a structured question. Its tool is named `ask_user_question`, which the shared user-blocking matcher did not recognize, so Vibe fell through to the trailing-question-mark text heuristic instead. Its `write_file` tool is also now recognized as a permission-gated edit. (#1109, #1087)
- History control rows that ask to be hidden are actually hidden. An author `display: flex` on `.history-ctl-row` outranked the user-agent `[hidden]` rule, so all three per-chart toggles were silently inert — and two rows shipped the `hidden` attribute in the markup, so the default page load mis-rendered too. (#1101, #1079)

**Changed / Docs / Testing**
- `monitoring-surface.md` documents all 10 agent adapters; it previously covered 4. (#1092, #1090)
- The structural claims in `monitoring-surface.md` are now enforced as census tests, so the doc cannot drift from the adapters it describes without a test failing. (#1107)
- Five stale source comments were corrected and their remote annotations dropped. (#1111, #1097)
- The four duplicated copies of the `sessionsDir()` env-override helper were extracted into one. (#1094, #1085)
- `/ir:triage` gained an Observability axis with a cost-tiered instrument plan — it asks whether a reported behavior can be reproduced and observed with what exists today, which is the gap that blocks an agent before verifiability does. (#1093, #1091)
- The 2026-07-15 agent-release sweep was recorded, and false claims it surfaced across the source and docs were corrected. (#1072, #1077)
...
- The Vibe checkpoint-rewind daemon premise is scoped to the TUI path in `replaydata`, matching the ACP-only reachability the rotation fix depends on. (#1110, #1089)
- Model alias map re-synced against codeburn upstream: no changes this cycle.

### Technical appendix

- **`pendingBackgroundAgentCount` guard release (#1098, #1076).** #1037 (v0.5.8) added the guard; its release condition was observing `pendingBackgroundAgentCount: 0`, which never occurs. Across 1,698 observed `turn_duration` lines the value `0` appears zero times, while `1`–`13` appear 176 times and the field is omitted 1,522 times — it is value-driven, not version-driven. The fix normalizes absent → `0` in `claudecode/parser.go`, scoped to `turn_duration` only. That scoping is load-bearing: `stop_hook_summary` shares the same `turn_done` branch but never carries the field (0 of 88 observed lines), so absence there means "no information", not "zero" — conflating them would clobber a live non-zero count on every stop-hook event and re-open #1036. Two tests exist purely to lock the scoping. Part 2 persists the count in `LedgerState` alongside `BackgroundProcs` (#445) and `LastEventType` (#649), because a restart resumes at `LastOffset` and processes zero lines, which left the guard inert on every restart. No schema bump.
- **In-place rewrite detection (#1120, #1104).** `TranscriptTailer.TailAndProcess` had one rotation check, `fileSize < t.lastOffset`. The fix hashes the up-to-512 bytes ending at `lastOffset` after each pass and re-hashes that window before trusting the offset on the next. The soundness argument is one line: an append never rewrites bytes below `lastOffset`, so a matching window means a genuine append and a mismatch means the consumed prefix was rewritten — routed through the same reset-and-replay-from-byte-0 path a shrink already used. A newline-boundary probe was rejected as insufficient (the prefix is corrupt whether or not the stale offset lands on a boundary); inode identity was rejected as the wrong signal (an in-place rewrite keeps the inode, and reading it needs build-tagged files on a platform-neutral package). Failure modes are asymmetric by design: a pre-#1104 ledger carrying an offset but no fingerprint, and an unreadable window, both fall back to historical resume-at-offset behavior — a false positive costs a full re-read, a false negative is what `main` did. The fingerprint is persisted in `LedgerState` so a rewrite landing while the daemon is down is caught on the next boot. Because `resetAccumulatorsForRotation` is the only caller of `ResetForRotation()`, #1063/#1078's `rotationResetter` opt-in inherited this blind spot exactly — those fixed what happens *on* a detected rotation, not detection itself.
- **`openToolCalls` on rotation (#1118, #1088).** #1088 was a verification task over four narrow relocation/prune edges, none known broken. Three proved not real and are now pinned by lock tests plus documented evidence: delete-then-create ordering during relocation; subagent relocation detection (unreachable); prune vs. cumulative totals. The fourth was real: `resetAccumulatorsForRotation` cleared every other per-file accumulator but not `openToolCalls`. Re-reading from byte 0 re-inserts every surviving `tool_use` by id and the map is id-keyed, so that much is idempotent — what it cannot fix is a call left open in the pre-rotation content, which no surviving line re-inserts or closes. Fixed with a regression test. The headline relocation finding from parent issue #1065 was already solved in #877 and is untouched.
- **Vibe rotation parser reset (#1078, #1063).** Vibe's parser tracks a token high-water mark (`lastPromptTokens`/`lastCompletionTokens`) derived from `meta.json`'s session-cumulative counters and emits each turn's delta. Previously unreachable because Vibe's `/rewind` always forked a new session directory — 2.19.1 made it reachable via the ACP path only: `_overwrite_messages_sync` (tempfile + `os.replace`) is gated on `inplace=True`, which the TUI does not pass (defaults to `inplace=False` → forks) but ACP does, and ACP shares the session directory. Without a parser-side reset the stale high-water mark computes a negative delta, both values clamp to zero, and `emitContribution`'s early return skips updating the mark — so every subsequent `turn_done` repeats the clamp. The fix adds an optional `rotationResetter` interface (`ResetForRotation()`) the tailer type-asserts for, a no-op for the six other adapters sharing the tailer. Vibe's implementation resets the token high-water mark and the `TodoReconciler` — the latter because its synthetic per-todo task IDs are assigned in lockstep with the tailer's `taskSeq`, which is already reset to 0 on rotation, so leaving `nextID`/`idByKey` stale would desync the two ID spaces. Deliberately not reset: `path` (the file does not change across an in-place rewrite) and `sidecarCache` (self-invalidates via its own mtime/size check).
- **Vibe `$VIBE_HOME` (#1108, #1095).** Settled against the installed `mistral-vibe` 2.19.1 package two independent ways — source read of `vibe/core/paths/_vibe_home.py` and executing that same installed code. Upstream honors the variable; the adapter's contradicting in-repo claim was wrong and the changelog was right.
- **Vibe `save_dir` (#1123, #1115).** `vibe/core/config/models.py:58-63` — `save_dir` defaults to `SESSION_LOG_DIR` (`$VIBE_HOME/logs/session`) and, when set, *replaces* the root rather than layering under it. `vibe/core/session/session_logger.py:56-76` writes `<save_dir>/<prefix>_<ts>_<id>/`, holding the `messages.jsonl` + `meta.json` pair the adapter watches. #1115 asked for a worth-fixing judgement first: `cli/cli.py:133-141` (`bootstrap_config_files`) writes a default `config.toml` on first run with `save_dir` already spelled out as a resolved absolute path, so the key sits in every installation's config. That same fact makes the fix low-risk — in the common case the configured value equals what Irrlicht already computed, so the read is a no-op. Without it, the fswatcher blocks in `waitForRoot` on a path that never appears, indistinguishable from "no vibe sessions exist".
- **Kiro `KIRO_HOME` (#1074).** Follows `1165dcdf` ("honor agent-CLI env vars for relocated session dirs", #349/#362), which wired the same fix into `codex` (`CODEX_HOME`), `claudecode` (`CLAUDE_CONFIG_DIR`), and `pi` (`PI_CODING_AGENT_SESSION_DIR`); kirocli was authored later (#590) and missed. `sessionsDir()` now reads `KIRO_HOME`, rejects non-absolute values with a log (same as siblings), and joins `sessions/cli` onto an absolute override; the default stays home-relative `.kiro/sessions/cli`. The false comment claiming "Kiro documents no env var that relocates the session root" — already false when written — is deleted. kirocli had no adapter tests at all; a table test modeled on `codex/adapter_test.go` is new. The `transcripts` permission's consent copy no longer hardcodes `~/.kiro/sessions/cli/`. Caveat carried forward from the PR: the `sessions/cli` append is **not** docs-confirmed — kiro.dev's settings reference says `KIRO_HOME` overrides the `~/.kiro` directory "used for … sessions" without stating whether sessions live directly under it or under a subdirectory. The append was implemented by analogy with `CODEX_HOME` (identical shape) rather than guessing a flat layout.
- **Vibe tool-name matchers (#1109, #1087).** Two shared matchers used exact equality and did not know Vibe's snake_case names. `ask_user_question` is Vibe's structured question tool (live in 2.19.1 `tools_available`), but the user-blocking set was `{AskUserQuestion, ExitPlanMode, question}` — the `question` entry matches a tool literally named `question`, not a substring — so Vibe never fired rule 1 (`NeedsUserAttention`). Fixed in both copies: `core/pkg/tailer/tailer_config.go` and `core/domain/session/metrics.go`. The duplication is kept deliberately — `isUserBlockingToolName`'s doc comment records that it is kept local to the tailer to avoid a domain-package import, and that rationale is intact (no production file in `core/pkg/tailer` imports `domain/session`; only its tests do). Both sites now carry an explicit KEEP-IN-SYNC cross-reference and twin tests pin both sets. Separately, `write_file` was added to `isPermissionGatedEditTool`: upstream's v2.14.0 `search_replace`→`edit` rename had silently opted `edit` into the OpenToolStalled heuristic with no change on our side, while `write_file` never matched `write`. Because that matcher is shared, every adapter under `core/adapters/inbound/agents/` was re-checked against the doc comment's "no adapter names a long-running tool with one of these spellings" claim.
- **History `[hidden]` (#1101, #1079).** `.history-ctl-row` sets an author `display: flex`, which outranks the UA `[hidden] { display: none }` rule, so every `el.hidden = true` flipped the property and left the row on screen — `syncHistoryRangeRow`, `syncGranularityRow`, and `syncDoraProjectRow` were all silently inert. `#history-dora-row` and `#history-granularity-row` ship the `hidden` attribute in `index.html`, so the default page load mis-rendered too; `#history-chart` had the same shape with an author `display: block`, so `syncHistoryMatrixVisibility`'s `canvas.hidden` never took effect. Fixed with two scoped hatches mirroring the existing `.history-matrix-scroll[hidden]` precedent — the one element that already worked because someone hit this exact bug there and patched it locally while the siblings were missed. `#history-chart[hidden]` needs the id to win the cascade (1,1,0 beats `#history-chart`'s 1,0,0). Deliberately not a global `[hidden] { display: none !important }` reset: that would change every element currently relying on the buggy behavior and wants its own sweep of `.hidden =` call sites first. The whole `platforms/web/` tree was swept — DOM `hidden` toggling happens exclusively in `historyTab.js` (12 call sites), everything else hides via `classList`; of the 12, the 4 fixed here were the only unhatched ones. This is a real visible change: rows users currently see start disappearing, per each helper's stated intent.
- **Activity chart gate (#1080, #1075).** A plain local display preference, `enableActivityChart`, default off, on both frontends. Unlike the task-eta (#558) and backchannel (#724) activations it sits beside in Advanced Settings, nothing is installed — no `ActivationClient`, no daemon reconcile, no `.onAppear` mirror; the daemon still serves `chart=state` unconditionally. Web: `SETTINGS_DEFAULTS` plus an Advanced Settings row with the existing `beta-badge`, split deliberately into two halves — `syncActivityChartVisibility()` hides `button[data-chart=state]` and rides `applySettings()` alongside the other reflect-onto-DOM toggles, while `leaveActivityChartIfSelected()` hangs off the per-key effect path next to `SOURCE_SETTING_KEYS` (a first draft had both in `applySettings()`, which meant editing `relayUrl` fired a history fetch). The button ships `hidden` in `index.html` so a default-off gate never flashes it. macOS: `@AppStorage("enableActivityChart")` plus `LeadingToggle(beta: true)`, with `HistoryTab.visible(activityEnabled:)` filtering the picker per the existing `visibleCharts` precedent. Both platforms reset the selection when the gate is turned off while Activity is live (web → `cost`, macOS → `.usage`); on macOS this is reachable in normal use since Settings is a separate window. A `.history-seg button` separator swap from `border-right`/`:last-child` to `border-left`/`:first-child` renders identically with all buttons shown but stops a hidden last button stranding a border against the fieldset edge.
- **`/ir:triage` Observability axis (#1093, #1091).** Verifiability names the *measurement*; Observability asks whether the *instrument* exists — ✓ Verifiability with ✗ Observability is the case the axis is built for. One deliberate deviation from #1091 as filed: a cost gate rather than a hard fail to `needs-info`. Low (instrument exists, just run it) and Medium (extend an existing surface — new `lifecycle.Kind`, metric field, snapshot scene) leave the verdict unchanged and land the plan in the brief as the ticket's first step; only High (the rig outweighs the fix) routes to `needs-info` plus a child issue. The skill's standing "never propose solutions" contract gets an explicit carve-out, stated in both the opening paragraph and "What to leave out" so the two cannot drift: triage may prescribe the instrument, never the fix. The altitude test: if your Instrument line would survive being pasted into a different fix for the same bug, it is the right altitude.
