...
**Added**
- **GitHub Copilot CLI is now a monitored agent** — Irrlicht's tenth adapter. Sessions are discovered from `~/.copilot/session-state/<session-id>/events.jsonl` (relocatable via `$COPILOT_HOME`), with the session id taken from the containing directory rather than the filename, which is constant. Copilot writes explicit turn boundaries, so `working`/`ready` ride on real records instead of a quiet-tail heuristic; permission prompts route to `waiting` from the transcript itself, with no hook installation required; and tokens and cost are live per assistant message rather than flat-zero until the agent exits. In-process subagents (`task`/`explore`) are counted on the parent, which is where Copilot records them — it creates no child session directories. Verified against CLI 1.0.77–1.0.78. (#1256)
- **[herdr](https://herdr.dev/) is now a supported terminal backend**, alongside tmux and kitty. A session running in a herdr pane can be typed into, interrupted and read back from the daemon — headless, with no macOS automation prompt, and reachable over the relay — by scripting `herdr pane send-text` / `send-keys` / `read` against the pane recorded in `$HERDR_PANE_ID`. The pane's server is addressed through `$HERDR_SOCKET_PATH`, which is herdr's documented equivalent of `tmux -S <socket>` and the only key that distinguishes one running server from another. Submitting input goes through `pane run` rather than a trailing carriage return, because a CR carried inside `send-text` stops submitting once the text wraps in the agent's composer — so a realistic prompt was typed and left unsent. Verified end to end against herdr 0.8.0 driving a real Claude Code TUI, plus a real-server test that boots its own isolated instance. (#1348)
...
- Open subagents are now counted from the parser that actually read the transcript. `agents.SubagentCounters` built one parser at wiring time and closed over it, while the metrics adapter builds a separate parser per transcript path — so any adapter keeping its count in parser state reported zero forever. Claude Code was immune by accident rather than design: its counter ignores the receiver, and its unit test passed for that same reason. (#1256)
- `irrlicht-focus` now resolves the daemon's port (`IRRLICHT_BIND_ADDR`, else the running daemon's `irrlichd.addr` file, else the default) instead of hardcoding `:7837`, so focusing a session monitored by a dev daemon on an alternate port no longer sends the request to production. (#1215)
...
[Unreleased]: https://github.com/ingo-eichhorst/Irrlicht/compare/v0.5.9...HEAD
