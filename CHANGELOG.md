...
### Added
- **A permission that is granted but not actually in force is now visible without opening Settings — on both macOS and the web dashboard.** The daemon re-runs every granted consent effect at startup; when one fails there, #1362 recorded the reason on that permission, but the only place it was ever rendered was the permission wizard. A user who never opened Settings saw nothing at all — permission reads "granted", no error anywhere, and the only symptom is that irrlicht quietly stopped detecting `waiting`. `GET /api/v1/permissions` now carries an aggregate (`unapplied_grants`), and both surfaces render it passively: a status strip in the macOS panel and a banner on the dashboard, each reading "N permissions are granted but not applied" with every cause spelled out beneath it and a **Review permissions** button. Deliberately quiet — it never opens the wizard by itself (the fail → wizard → retry → fail loop #1362 avoided), never touches the menu-bar attention icon, which stays reserved for consent you must answer now, and has no dismiss control: it goes away by being fixed. The causes stay named rather than collapsing into one number — an install that failed and a refusal because the agent's CLI is below its version floor read differently, and the refusal still carries its own advice. (#1385)
...
### Fixed
- **`irrlichd --uninstall-task-eta` now sticks — the managed blocks no longer come back in your own `CLAUDE.md` on the next daemon start.** The command removed irrlicht's `BEGIN`/`END`-delimited blocks from `~/.claude/CLAUDE.md` and did nothing else, so `claude-code/instructions` stayed `granted` in the permission store — and the daemon re-runs every granted consent effect at startup, which wrote the blocks straight back into a file the user writes prose in. It is the same #570 regression #1425 fixed on `--uninstall-hooks`, on the flag next door, and it reverted on restart rather than within five minutes, whether or not a daemon was running at the time. The command now records the opt-out and sends the same consent-reload nudge #1425 added, so a daemon that is already running drops the permission immediately instead of at its next restart. Both flags are one shared "uninstall, deny, signal" path now, over a projection of the consent catalog narrowed to the permission each one names — so neither can revoke the other's capability, and neither can drift away from the other again. With no daemon running it still exits 0 and prints "No running daemon to notify": an undeliverable signal is not an error, because the recorded decision takes effect at the next start either way. Nothing about which bytes are removed changed — only whole, well-formed sentinel pairs are cut, and surrounding content is preserved byte-for-byte. (#1437)
- **A dev or test daemon started with `IRRLICHT_PERMISSION_MODE=grant-all` no longer rewrites your real agent configs.** `grant-all` answers every declared permission on your behalf, which runs every install closure — so a daemon started by hand on a throwaway port (the ordinary way to test anything consent-gated) repointed `~/.claude/settings.json` and `~/.codex/hooks.json` at that port and left them there when it died. The observed damage on one machine was 25 references across three dead ports in `settings.json` and 3 more in `codex/hooks.json`, with a live daemon on 7837: permissions reading `granted`, entries present, and nothing arriving. Two of those ports predated the incident, so it had been happening quietly for a while. A `grant-all` daemon now refuses any install that writes a shared config file outside its own `IRRLICHT_HOME`, with an error naming the file, and the permission shows as "granted but NOT applied" with that reason. Note that `IRRLICHT_HOME` alone never isolated these files — they follow `$HOME` — which is why setting it was not already protection. `IRRLICHT_ALLOW_SHARED_CONFIG_WRITES=1` opts back in for someone who genuinely wants it; the onboarding recording rig sets it and is entitled to, because it snapshots every managed file and restores it on exit (#1383). Two consequences worth knowing: `ir:test-mac`'s separate mode no longer installs hooks, which was the damage rather than a feature, and re-recording via `of record run --attach` now requires you to back the files up and set the variable yourself — the rig refuses up front rather than recording a fixture in which the adapter merely looks incapable. (#1449)
- **A mistyped `irrlichd` flag now fails instead of silently starting a misconfigured daemon.** `irrlichd` had never parsed its command line: it scanned `os.Args` for a handful of known flags in turn and fell through to starting the daemon for anything else. So `irrlichd --recrod` started a daemon with recording *off* and said nothing — the worst available shape, because it looks like success, and `--record` is the difference between capturing an onboarding session and not. The seven accepted flags are now an explicit allow-list checked before dispatch, and an unrecognized flag prints a message naming it to stderr and exits 2 without starting anything (a `--flag=value` form too, since none of them takes a value). Every existing invocation is unchanged — the no-argument form the macOS app, the systemd unit and the LaunchAgent all use, `--record`, `--version`, `-v`, `--diagnose`, `--uninstall-hooks`, `--print-managed-files`, `--uninstall-task-eta`, and the beacon's `--version hook-post <adapter>` guarded form, whose leading `--version` still selects the beacon rather than the version banner. This retires the workarounds #1357, #1373 and #1416 each had to build around the old fall-through. (#1417)
...
[Unreleased]: https://github.com/ingo-eichhorst/Irrlicht/compare/v0.5.10...HEAD
