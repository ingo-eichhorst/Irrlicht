...
test_success() {
  ...
  got="$(dsh_await_child_turn_end "$root" "$parent" $(( $(date +%s) + 1 )))" || { fail success; rm -rf "$root"; return; }
  [[ "$got" == "$root/$child/session.v3.jsonl.zstd" ]] && pass success || fail success
  rm -rf "$root"
}

test_timeout() {
  local root parent child out status
  ...
  set +e
  out="$(dsh_await_child_turn_end "$root" "$parent" "$(date +%s)" 2>&1)"; status=$?
  set -e
